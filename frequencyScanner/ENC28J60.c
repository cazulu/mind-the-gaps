...
void MACInit() {
	...
	BankSel(ERXFCON);
	WriteReg((BYTE)ERXFCON,ERXFCON_UCEN | ERXFCON_CRCEN | ERXFCON_MCEN | ERXFCON_BCEN);
...
//	if(ReadETHReg((BYTE) EREVID).Val!=0x06){
//		MACInit();
//	}
	...
	if (ENCRevID == 0x05u || ENCRevID == 0x06) {
...
//    //TODO: Quick hack to save the ENC28J60 from a dead SPI
//    	//If the version is incorrect, we assume a dead SPI
//    	//and trigger a hardware reset
//    	ETH_RST_OUT &= ~ETH_RST;
//    	//According to datasheet, T_RSTLOW=400ns
//    	//but we wait 1ms just to be really sure
//    	__delay_cycles(8000);
//      	ETH_RST_OUT |= ETH_RST;
    ...
    __delay_cycles(8000); //-> Delay 1ms with a 8Mhz CPU clock
